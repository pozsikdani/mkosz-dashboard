...
def _is_pbp(cfg):
    """Check if this team's competition uses PBP event data."""
    ...
    return conn.execute(f"""
        WITH kg AS (
            SELECT m.gamecode, m.match_date,
                   CASE WHEN m.team_a_name LIKE ? THEN 'A' ELSE 'B' END as kg_team,
                   CASE WHEN m.team_a_name LIKE ? THEN 'H' ELSE 'V' END as hv,
        ...
        SELECT kg.gamecode, kg.match_date, kg.hv, kg.opponent, kg.kg_score, kg.opp_score,
               ...
               pgs.personal_fouls, pgs.is_starter
        FROM kg
        LEFT JOIN player_game_stats pgs
            ON pgs.gamecode = kg.gamecode
...
def get_quarter_stats_pbp(conn, cfg, tp, player_id, hv_filter=None, phase_filter=None):
    ...
    return conn.execute(f"""
        ...
        FROM pbp_events e
        JOIN kg ON e.gamecode = kg.gamecode AND e.team = kg.kg_team
...
def get_team_stats_pbp(conn, cfg, tp, hv_filter=None, phase_filter=None):
    ...
    # Build match filter
    if hv_filter == 'H':
        _mf = "m.team_a_name LIKE ?"
        _mp = (tp,)
    elif hv_filter == 'V':
        _mf = "m.team_b_name LIKE ?"
        _mp = (tp,)
    else:
        _mf = "(m.team_a_name LIKE ? OR m.team_b_name LIKE ?)"
        _mp = (tp, tp)
    ...
    d["top_scorers"] = conn.execute(f"""
        ...
        per_game AS (
            SELECT e.player_name, e.gamecode,
                   SUM(CASE WHEN e.is_scoring=1 THEN e.points ELSE 0 END) as pts
            FROM pbp_events e JOIN kg ON e.gamecode=kg.gamecode AND e.team=kg.t
            WHERE e.player_name IS NOT NULL
            GROUP BY e.gamecode, e.player_name
        )
...
def generate_html(player_data, game_log, quarter_stats, opp_stats, tech, unsport, cfg, training_att=None, bundles=None):
    ...
    for val in opp_ppg_data:
        if val >= ppg * 1.2:
            # Kiemelkedő ellenfél elleni átlag → pozitív
            opp_colors_bg.append("rgba(0,184,148,0.6)")
            opp_colors_border.append("#00b894")
        elif val >= ppg * 0.7:
            # Átlagos teljesítmény → semleges
            opp_colors_bg.append("rgba(139,141,160,0.5)")
            opp_colors_border.append("#8b8da0")
        else:
            # Gyenge teljesítmény → negatív
...
<div class="dashboard">
  ...
  <div class="grid grid-2 mb20">
    ...
    <div class="card">
      ...
      <div style="margin-top:20px;">
        <div class="shot-row">
          <div class="shot-label" style="color:#c0c2ce">3FG</div>
          <div class="shot-bar-outer"><div class="shot-bar-inner" style="width:{bar_3fg}%;background:#c0c2ce;color:#1a1b20;">{fg3} db</div></div>
          <div class="shot-pct" style="color:#c0c2ce">{pts_3fg} pt</div>
        </div>
        <div class="shot-row">
          <div class="shot-label" style="color:#8b8da0">2FG</div>
          <div class="shot-bar-outer"><div class="shot-bar-inner" style="width:{bar_2fg}%;background:#8b8da0;">{fg2} db</div></div>
          <div class="shot-pct" style="color:#8b8da0">{pts_2fg} pt</div>
        </div>
        <div class="shot-row">
          <div class="shot-label" style="color:#5a5c6a">FT</div>
          <div class="shot-bar-outer"><div class="shot-bar-inner" style="width:{bar_ft}%;background:#5a5c6a;">{ft_m}/{ft_a}</div></div>
          <div class="shot-pct" style="color:#5a5c6a">{pts_ft} pt</div>
        </div>
      </div>
  ...
  <div class="card mb20">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;font-size:0.85rem;line-height:1.7;">
      <div>
        <div style="color:var(--green);font-weight:700;margin-bottom:6px;">Erősségek</div>
        <ul style="padding-left:18px;color:var(--text-dim);">
          {"".join(f'<li><span style="color:#fff;font-weight:600;">{s}</span></li>' for s in strengths)}
        </ul>
      </div>
      <div>
        <div style="color:var(--red);font-weight:700;margin-bottom:6px;">Fejlesztendő területek</div>
        <ul style="padding-left:18px;color:var(--text-dim);">
...
new Chart(document.getElementById('scoringTrend').getContext('2d'), {{
  ...
  data: {{
    ...
    datasets: [{{
      ...
      borderColor:'#8b8da0', backgroundColor:'rgba(139,141,160,0.08)',
...
new Chart(document.getElementById('shotPie').getContext('2d'), {{
  ...
  data: {{
    ...
    datasets:[{{ data:[{pts_3fg},{pts_2fg},{pts_ft}], backgroundColor:['#c0c2ce','#8b8da0','#5a5c6a'], borderColor:'#151518', borderWidth:3, hoverOffset:8 }}]
  }},
...
new Chart(document.getElementById('quarterChart').getContext('2d'), {{
  ...
  data: {{
    ...
    datasets:[{{
      ...
      backgroundColor:['rgba(139,141,160,0.6)','rgba(139,141,160,0.6)','rgba(139,141,160,0.6)','rgba(139,141,160,0.6)'],
      borderColor:'#8b8da0', borderWidth:2, borderRadius:6,
    }},{{
      label:'3FG bedobva', data:{json.dumps(q_3fg_data)},
      backgroundColor:'rgba(192,194,206,0.6)', borderColor:'#c0c2ce', borderWidth:2, borderRadius:6,
    }}]
  }},
  options: {{
    responsive:true, maintainAspectRatio:false,
...
def get_team_stats(conn, cfg, tp, hv_filter=None, phase_filter=None):
    ...
    d["scenarios"] = conn.execute(f"""
        ...
        halves AS (
            ...
            FROM kg JOIN quarter_scores qs ON qs.gamecode=kg.gamecode
            WHERE qs.quarter IN ('1','2','3','4')
            GROUP BY kg.gamecode
        )
    ...
    for label, is_team_val in [("runs_for", 1), ("runs_against", 0)]:
        ...
        rows = conn.execute(f"""
            ...
            made AS (
                ...
                FROM scoring_events se JOIN kg ON se.gamecode=kg.gamecode WHERE se.made=1
            ),
            with_rid AS (
                SELECT *,
                       SUM(CASE WHEN is_team={opp_val} THEN 1 ELSE 0 END) OVER (
...
def get_calendar_data_db(conn, cfg, tp):
    ...
    rows = conn.execute(f"""
        SELECT m.match_date, m.match_time,
               ...
               CASE WHEN m.team_a_name LIKE ? THEN m.team_b_name ELSE m.team_a_name END as opponent,
               CASE WHEN m.team_a_name LIKE ? THEN m.score_a ELSE m.score_b END as our_score,
               CASE WHEN m.team_a_name LIKE ? THEN m.score_b ELSE m.score_a END as opp_score,
               m.gamecode
        FROM matches m
...
<style>
  ...
  * {{ margin:0; padding:0; box-sizing:border-box; }}
  ...
  .m-hv {{ font-size:0.75rem; font-weight:700; color:var(--text); }}
...
def generate_match_page(details, cfg, team_key):
    ...
    max_y = max((p["y"] for p in kg_prog_pts + opp_prog_pts), default=0)
...
const quarterMarkerPlugin = {{
  ...
  afterDatasetsDraw(chart) {{
    ...
    qMarkers.forEach((m, idx) => {{
      ...
      const by = top + 4;
      // Háttér (semleges sötétszürke)
      ctx.fillStyle = 'rgba(45,47,56,0.95)';
      ctx.beginPath();
...
const kgDs = {{label:{json.dumps(kg_short)}, data:[], borderColor:'#00b894', backgroundColor:'rgba(0,184,148,0.05)', borderWidth:2.5, pointRadius:0, pointHoverRadius:4, stepped:'after', fill:false}};
const oppDs = {{label:{json.dumps(shorten_opponent(details["opp_team_name"]))}, data:[], borderColor:'#8b8da0', backgroundColor:'rgba(139,141,160,0.05)', borderWidth:2.5, pointRadius:0, pointHoverRadius:4, stepped:'after', fill:false}};
const datasetsOrdered = {'[kgDs, oppDs]' if details["kg_is_home"] else '[oppDs, kgDs]'};

// Animáció: negyed-határok seq értékei (pause pontok)
const qBoundarySeqs = qMarkers.filter((_,i) => i < qMarkers.length - 1).map(m => m.x);
let animVisibleMarkers = 0;

const quarterMarkerAnimPlugin = {{
  id: 'quarterMarkerAnim',
  afterDatasetsDraw(chart) {{
    const {{ctx, chartArea: {{top, bottom}}, scales: {{x, y}}}} = chart;
    if (!qMarkers || !qMarkers.length) return;
    ctx.save();
    const visibleCount = Math.min(animVisibleMarkers, qMarkers.length - 1);
    for (let idx = 0; idx < visibleCount; idx++) {{
      const m = qMarkers[idx];
      const px = x.getPixelForValue(m.x);
      ctx.strokeStyle = 'rgba(255,255,255,0.18)';
      ctx.lineWidth = 1;
      ctx.setLineDash([4, 4]);
...
const progressChart = new Chart(document.getElementById('progressChart').getContext('2d'), {{
  ...
  plugins: [quarterMarkerAnimPlugin],
  options:{{
    animation: false,
    responsive:true, maintainAspectRatio:false,
    ...
    scales:{{
      y:{{beginAtZero:true, suggestedMax:{max_y}, grid:{{color:'rgba(255,255,255,0.04)'}}}},
      x:{{
...
// Progresszív animáció: adatpontok megjelenítése negyed-szünetekkel
(function animateChart() {{
  const SPEED = 35;
  const Q_PAUSE = 1500;
  const kgHome = {'true' if details["kg_is_home"] else 'false'};
  const ds0 = kgHome ? kgProg : oppProg;
  const ds1 = kgHome ? oppProg : kgProg;
  let i = 0;
  let qIdx = 0;

  function addNext() {{
    if (i >= ds0.length) return;
    datasetsOrdered[0].data.push(ds0[i]);
    if (i < ds1.length) datasetsOrdered[1].data.push(ds1[i]);
    progressChart.update('none');
    i++;
    const curSeq = ds0[i-1] ? ds0[i-1].x : 0;
    if (qIdx < qBoundarySeqs.length && curSeq >= qBoundarySeqs[qIdx]) {{
      animVisibleMarkers = qIdx + 1;
      progressChart.update('none');
      qIdx++;
      setTimeout(addNext, Q_PAUSE);
    }} else {{
      setTimeout(addNext, SPEED);
    }}
  }}
  // IntersectionObserver: animáció indul amint a chart bármennyire is látható
  const el = document.getElementById('progressChart');
  let started = false;
  function startOnce() {{ if (!started) {{ started = true; addNext(); }} }}
  if (el && 'IntersectionObserver' in window) {{
    const obs = new IntersectionObserver((entries) => {{
      if (entries[0].isIntersecting) {{ obs.disconnect(); startOnce(); }}
    }}, {{threshold: 0}});
    obs.observe(el.closest('.card') || el);
  }}
...
<style>
  ...
  * {{ margin:0; padding:0; box-sizing:border-box; }}
  ...
  .nm-matchup {{
    ...
    margin:6px 0 4px; position:relative; z-index:1;
  }}
  .nm-time {{ font-size:1.4rem; font-weight:900; color:var(--text); letter-spacing:-0.5px; }}
  .nm-vs {{ font-size:0.9rem; color:var(--text-dim); font-weight:600; }}
  .nm-opp {{ font-size:1.15rem; font-weight:800; color:var(--text); letter-spacing:-0.3px; }}
  .nm-venue {{
  ...
  @media(max-width:600px) {{
    ...
    .next-match-card {{ padding:12px 14px; }}
    .nm-time {{ font-size:1.25rem; }}
    .nm-opp {{ font-size:1.05rem; }}
    .nm-date {{ font-size:0.78rem; }}
...
def generate_team(team_key):
    ...
    src_label = f"{'PBP' if pbp else 'Scoresheet'} ({cfg['comp_code']})"
    ...
    if not roster:
        ...
        _team_stats_fn = get_team_stats_pbp if pbp else get_team_stats
